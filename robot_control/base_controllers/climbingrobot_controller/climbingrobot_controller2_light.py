...
class ClimbingrobotController(BaseControllerFixed):
    ...
    def loadModelAndPublishers(self, xacro_path=None):
        ...
        if self.SAVE_BAG:
            self.recorder = RosbagControlledRecorder(bag_name = "climbing_robot_terrain_single_traj_plot.bag", record_from_startup_=False)
    ...
    def initOptim(self, p0, pf):
        ##offline optim vars
        self.Fleg_max = 150.
        self.Fr_max = 60.  # had to increas because of slopes  it used tp be 90
    ...
    def stateMachineLoop(self):
        ...
        if ( p.stateMachine == 'start_jump') and (p.time >= p.startJump):
            ...
            if p.PAPER and p.jumpNumber == 0:
                ...
                p.initOptim(p.targetPos_paper[0], p.desired_target[1])
                ...
                # p.initOptim(p.targetPos_paper[1], p.desired_target[2])
                # p.total_ref_com = np.concatenate((p.total_ref_com, p.ref_com), axis=1)
                # p.targetPos_paper.append(p.ref_com[:, -1])
        ...
        if (p.stateMachine == 'thrusting'):
            ...
            if (p.time > p.end_thrusting):
                ...
                if  p.landing:
                    ...
                    p.q_des[p.leg_index] = np.array([-1.57, 0.0, 0.2])
                else:
                    # retract leg

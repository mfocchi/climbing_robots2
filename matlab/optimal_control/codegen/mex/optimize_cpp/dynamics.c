...
void dynamics(real_T t, const real_T x[6], real_T Fr_l, real_T Fr_r, const
              real_T Fleg[3], real_T params_m, real_T params_b, const real_T
              params_p_a1[3], const real_T params_p_a2[3], real_T params_g,
              real_T params_T_th, real_T dxdt[6])
{
  real_T b_x[9];
  real_T c_x[9];
  real_T n_bar_tmp[3];
  ...
  scale = 3.3121686421112381E-170;
  for (p1 = 0; p1 < 3; p1++) {
    x_tmp = n_par[p1] / y * Fr_l + p[p1] / t3 * Fr_r;
    p[p1] = x_tmp;
  ...
  t1 = rope2_axis[2];
  for (p1 = 0; p1 < 3; p1++) {
    dxdt[p1 + 3] = (b_x[p1] * x_tmp + b_x[p1 + 3] * t3) + b_x[p1 + 6] * t1;
  }
}
